...
{{/if}}
								...
								<div class="form-group row">
									...
									{{elseif $i.input eq 'textarea' or $i.input eq 'cms'}}
										...
										<input type="hidden" name="tagged[]" value="{{$myId}}" />
											{{assign var="cmsfound" value=$myId}}
										{{/if}}
			...
			<script type="text/javascript">
				...
				//		_loadFields(value[1]);
									...
									{{elseif $i.input eq 'number'}}
										<input type="number" class="form-control" spellcheck="false" name="{{$myId}}" id="{{$myId}}" value="{{$object_row.$myId}}" validation="{{if !$smarty.request.id}}{{$i.validation}}{{/if}}" />
									{{else}}
