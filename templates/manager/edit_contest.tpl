					...
					$("#{{$myId}}").css('width', '100px').datepicker({
								...
								<div class="col-12">
									<table class="table table-striped table-bordered" id="tblRules">
										...
										<tr class="d-none firstTr">
											<td><input type="number" class="form-control" name="min[]" value="{{$v.min}}" style="width: 100px;" /></td>
											<td><input type="number" class="form-control" name="max[]" value="{{$v.max}}" style="width: 100px;" /></td>
											<td><input type="number" class="form-control" name="amount[]" value="{{$v.amount}}" style="width: 100px;" /></td>
											<td><a href="javascript: void(0);" onclick="deleteRule($(this))"><i class="fa fa-trash"></i></a></td>
										</tr>
										{{if $rules|@count}}
											{{foreach from=$rules key=k item=v}}
											<tr>
												<td><input type="number" class="form-control" name="min[]" value="{{$v.min_rank}}" style="width: 100px;" /></td>
												<td><input type="number" class="form-control" name="max[]" value="{{$v.max_rank}}" style="width: 100px;" /></td>
												<td><input type="number" class="form-control" name="amount[]" value="{{$v.amount}}" style="width: 100px;" /></td>
												<td><a href="javascript: void(0);" onclick="deleteRule($(this))"><i class="fa fa-trash"></i></a></td>
											</tr>
											{{/foreach}}
						...
						<script>
							...
							function getRuleAmount(){
								...
								$('table#tblRules input[name*="amount"]').each(function(){
									var amount = parseInt($(this).val());
									if(amount)
										total += amount;
								});
								var remainingAmt = price - total;
								console.debug(remainingAmt);
								return remainingAmt;
							}
